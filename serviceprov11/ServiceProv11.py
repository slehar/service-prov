...
# Open log file
if logging:
    cfg.logFilename = 'ServiceProv11.log'
    if os.path.exists(cfg.logFilename):
        os.remove(cfg.logFilename)
    cfg.fp = open(cfg.logFilename,'w')
    cfg.fp.write('Log file ServiceProv11.py %s\n\n'%datetime.datetime.now().strftime('%Y/%m/%d %H:%M:%S'))
    cfg.fp.close()

# function writeLog
'''
def writeLog(msg):
    cfg.fp = open(logFilename,'a')
    cfg.fp.write(msg)
    cfg.fp.close()
'''
    ...
    if checkService:
        ...
        if not agent['enrolled']:
            ...
            if probEnroll > random():
                ...
                if logging:
                    log.writeLog('Enrolling agent %d\n'%agent['id'])
            ...
            if random() > .25: # Randomize every other time to break sync
                ...
                if agent['treating'] == False:
                    ...
                    if agent['treatNo'] >= standardSched+1:
                        log.writeLog('  treatNo = %d standardSched = %d\n'%(agent['treatNo'], standardSched))
                        if logging: log.writeLog('Un-enroll agent %d treatment done\n'%agent['id'])
                        ...
                        for indx, sched in enumerate(schedList):
                            if sched[0] == int(agent['id']):
                                ...
                                break
                        if logging: log.writeLog('  agent %d treatment %d ON\n'
                                           %(agent['id'], agent['treatNo']))
                ...
                # Otherwise if patient being treated turn it off
                    ...
                    if logging: log.writeLog('  agent %d treatment %d OFF\n'
                                        %(agent['id'], agent['treatNo']))

...
def calculate_distribution():
    ...
    PTSD_sx = -2.113296 +                    \
              ...
              (1.197957  * isYoung) +         \
              (0.964156  * isOld) +           \
              (-0.011890 * isWhiteHispanic) + \
              (0.394813  * isBlackNonHisp) +  \
              (0.082708  * isBlackHispanic) + \
              (0.001690  * isOtherNonHisp) +  \
              (0.734497  * isOtherHispanic) + \
              (1.315114  * hasIncome1) +      \
              (1.228715  * hasIncome2) +      \
              (0.835060  * hasIncome3) +      \
              (0.951817  * hadStressors) +    \
              (0.363878  * nSandyTraumas)
    ...
    print 'exp(PTSD_sx)      = %f'%np.exp(PTSD_sx)
    axText.clear()
    ...
    axText.set_xticks([])
    axText.set_yticks([])
    axText.text(.3, .4, 'exp(PTSD_sx) = %f'%np.exp(PTSD_sx), size=14)    
    ...
    axTime.axis((t - plotWidth, t, 0., 15))
                ...
                for incomeKey in ('80-150k', '40-80K ', '< 40K'):
                    for hadStressorsKey in ('no stress', 'stress'):
                        for nSandyTraumasKey in ('0', '1', '2'):
                            ...
                            writelog.write(
                            '%s \t %s \t %s \t %s \t %s \t %s \t %r '%(
                sexKey, ageKey, raceKey, ethnKey, incomeKey, hadStressorsKey, nSandyTraumasKey))
                            writelog.write('\t%5.2f\n'%np.exp(PTSD_sx))
